...
pois0 <- glm(COUNT ~ DESCRIPTION + MONTH + DAY_OF_WEEK + DAY + HOUR, data = FireBrigadeCallOuts, family = poisson)
step(FitStart, direction = "forward", scope = formula(pois0))

FitStart = MASS::glm.nb(COUNT ~ 1, data = FireBrigadeCallOuts)
nb0 <- MASS::glm.nb(COUNT ~ DESCRIPTION + MONTH + DAY_OF_WEEK + DAY + HOUR, data = FireBrigadeCallOuts)
step(FitStart, direction = "forward", scope = formula(nb0))
nb1 <- MASS::glm.nb(COUNT ~ DESCRIPTION + MONTH + DAY_OF_WEEK + DAY + HOUR + TEMPERATURE + WIND_SPEED + PRECIPITATION + CLOUD_COVER, data = FireBrigadeCallOuts)
step(FitStart, direction = "forward", scope = formula(nb1))
nb2 <- MASS::glm.nb(COUNT ~ DESCRIPTION + MONTH + DAY_OF_WEEK + DAY + HOUR + EVENT, data = FireBrigadeCallOuts)
